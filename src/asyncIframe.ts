...
import { Emit, EventHandlerMap, OnOff, createEmitter } from './events'

/** 
  An iframe that loads its content asynchronously and establishes
  a communication channel via postMessage and a handshake.
**/
export default class AsyncIframe {
  ...
  constructor(source: string, container: Element, origin: string, eventHandlerMap?: Partial<EventHandlerMap>) {
    ...
    this._id = `iframe_${Math.random().toString(36).substring(2, 15)}`
  ...
  async postMessage<RequestType = unknown | any, ResponseType = unknown | any>(
    ...
    const messageId = `message_${Math.random().toString(36).substring(2, 15)}`
  ...
  private async _createIFrameElementAsync(source: string, container: Element) {
    const iframeElementPromise = new Promise<HTMLIFrameElement>((resolve, reject) => {
  ...
  private async _initiateHandshake(iframeElement: HTMLIFrameElement): Promise<void> {
    try {
...
export function getOrigin(url: string): string {
  ...
  return (
    ...
    (a.port !== '80' && a.port !== '443' && a.port !== '0' && a.port ? `:${a.port}` : '')
  )
}
