...
export default class ModalCheckout extends AbstractIframeCheckout<ModalCheckoutInstanceOptions> {
  ...
  /** The default transition CSS value for the overlay animation. */
  static readonly DEFAULT_TRANSITION = `all ${ModalCheckout.ANIMATION_DURATION}ms cubic-bezier(0.25, 0.8, 0.25, 1)`
  ...
  async show(event?: MouseEvent): Promise<void> {
    ...
    iframeContainer.style['-webkit-transform'] = iframeContainer.style.transform
  ...
  private _createFocusTrap(): HTMLDivElement {
    ...
    focusTrap.addEventListener('focus', () => (this._container as HTMLDivElement).focus())
...
function createAndApplyStyleSheet(cspNonce?: string) {
  ...
  addRuleToStyleSheet(`.${Style.IframeContainer}, .${Style.OverlayContainer}, .${Style.Backdrop}`, [
    'width: 100vw',
    'height: 100vh',
  ])

  addRuleToStyleSheet(`.${Style.OverlayContainer}`, ['z-index: 999999', 'position: fixed'])

  addRuleToStyleSheet(`.${Style.OverlayContainer}, .${Style.Backdrop}`, ['top: 0', 'left: 0'])

  addRuleToStyleSheet(`.${Style.Backdrop}`, ['background: rgba(0, 0, 0, 0.5)', 'position: absolute'])
  ...
  addRuleToStyleSheet(
    `${overlayEnter}, ${overlayLeave}, ` +
      ...
      `${overlayEnter} iframe, ` +
      `${overlayLeave} iframe`,
    [`transition: ${ModalCheckout.DEFAULT_TRANSITION}`, `-webkit-transition: ${ModalCheckout.DEFAULT_TRANSITION}`],
  )

  addRuleToStyleSheet(`${overlayEnter} iframe, ` + `${overlayLeaveActive} iframe`, [
    'opacity: 0',
    'transform: scale(0.1)',
  ])

  addRuleToStyleSheet(`${overlayEnter} .${Style.Backdrop}, ` + `${overlayLeaveActive} .${Style.Backdrop}`, [
    'opacity: 0',
  ])
...
function addRule(styleSheet: CSSStyleSheet) {
  return (selector: string, properties: Array<string>) => createRule(styleSheet, selector, properties)
}

function createRule(styleSheet: CSSStyleSheet, selector: string, properties: Array<string>) {
