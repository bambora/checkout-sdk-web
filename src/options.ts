...
export const DEFAULT_REDIRECT_OPTIONS: Readonly<RedirectCheckoutInstanceOptions> = {
...
export function mapOptionsToServerSideOptions(options: CheckoutInstanceOptions): PaymentWindowServerSideOptions {
  const { ui, language } = options
  return { ui, language }
}

/** Converts a client-side options object to a base64 string. */
export function optionsToBase64(options: Partial<PaymentWindowClientSideOptions>): string {
...
export interface ModalCheckoutInstanceOptions extends CommonCheckoutInstanceOptions {
  /**
   * Set a Content-Security-Policy nonce property for the inline stylesheet
...
export type IframeCheckoutInstanceOptions = InlineCheckoutInstanceOptions | ModalCheckoutInstanceOptions

/** Options generic for Checkout experiences. */
export type CheckoutInstanceOptions = RedirectCheckoutInstanceOptions | IframeCheckoutInstanceOptions
